...
//		SetIterateMode(REGULAR_ITERATE_AND_COMMS_DRIVEN_MAIL);
	...
	bool OnConnectToServer(){
		...
		Register("ACOMMS_RECV",0.0); // On Riptide
//		Register("ACOMMS_XMIT",0.0); // In simulation
	...
	bool OnNewMail(MOOSMSG_LIST & Mail){
		...
		for(q=Mail.begin();q!=Mail.end();q++){
			...
			if(q->GetKey()=="ACOMMS_RECV")   { acoms_ = q->GetString();
											  ...
											  cout <<"Responding to acoms\n";

											  Notify("ACOMMS_XMIT", "Come on sucker, lick my battery", MOOSLocalTime());
											}

/*			// In simulation
			if(q->GetKey()=="ACOMMS_XMIT")   { acoms_ = q->GetString();
												cout << q->GetKey() <<": " << q->GetString() <<endl;
											  Write2File();
											  acoms_ = "";
											  ...
											  Notify("ACOMMS_RECV", "Come on sucker, lick my battery", MOOSLocalTime());
												}
*/
			}
...
int main(int argc, char * argv[]){
	...
	string app_name = P.GetFreeParameter(1, "acoms2");
